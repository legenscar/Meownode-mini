# Meownode-mini
a compact meshtastic/meshcore 3D printable case for the Heltec t114 measuring 70mm x 47mm x 25mm. Includes space for a L76K GNSS module. requires a right angle IPEX to SMA adapter. 

<img width="400" height="540" alt="image" src="https://github.com/user-attachments/assets/a329a515-b6bb-455f-93c3-47eb6f788d82" />

## Required things:
- 3x M3x7x4 (7mm long) brass heat inserts
- 3x M3 screws (no countersunk)
- 1x L76K GPS module (optional)
- 1x heltec t114 board (with screen)
- 1x IPEX to SMA adapter (right angle)
- 1x antenna (base diameter <14mm)
- 1x battery (smaller than 10mm x 20mm x 50mm)
- 1x double sided tape

## Printing 
I suggest printing with a sturdier material like PETG or ABS.

## Assembly 
refer to the pictures for cable routing. When assembling, make sure not to pinch any cables. I glued the battery down with double sided tape so it doesnt rattle inside.

<img width="1080" height="810" alt="image" src="https://github.com/user-attachments/assets/bbef0b05-5d89-42ae-befa-113b4b904f8e" />
<img width="1440" height="1080" alt="first-node-finished-v0-1xn8az46niph1" src="https://github.com/user-attachments/assets/17801a6a-dc6b-49a0-9b8c-5f4645d17ab8" />


## Notes
tolerances vary from printer to printer, filament to filament. I cannot gurantee that this model will work with your printer, and some sanding might be required. Your final print may not look like the ones in the picture due to updates etc.. Feel free to modify the files to your hearts content. Not for commercial use. 


## License
Copyright © Legenscar.

This repository is licensed under the Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International license. See `LICENSE` or https://creativecommons.org/licenses/by-nc-sa/4.0/.
